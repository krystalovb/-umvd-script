// ==UserScript==
// @name         UMVD Rivera Lime - Clean & Fix
// @namespace    https://forum.blackrussia.online
// @version      22.0
// @description  Удалена норма, возвращена подпись, добавлен авто-исправитель ошибок.
...
(function() {
    ...
    // --- ИСПРАВИТЕЛЬ ОШИБОК (Форматирование ника) ---
    function fixNick(nick) {
        if (!nick) return "";
        return nick.trim()
            .replace(/\s+/g, '_') // Заменяет пробелы на нижнее подчеркивание
            .split('_')
    ...
    function showModal({ title, options = null, isSettings = false, inputPlaceholder = "" }) {
        return new Promise((resolve) => {
            ...
            if (isSettings) {
                ...
                <input id="set-sign" type="text" placeholder="Ваша Подпись (напр. Rivera)" value="${getSetting('riv_sign', '')}" style="width:100%; background:#16161e; border:1px solid #3b82f644; border-radius:8px; padding:10px; color:#fff; margin-bottom:10px;">
            ...
            m.querySelector('#m-confirm')?.addEventListener('click', () => { 
                ...
                if(inputPlaceholder === 'Nick_Name') val = fixNick(val); // Применяем исправитель
                resolve(val); 
                m.remove(); 
            });
    ...
    function createUI() {
        ...
        panel.innerHTML = `
            ...
            <button id="r-set" style="background:none; border:1px solid #444; color:#64748b; font-size:9px; padding:8px; border-radius:6px; margin-top:5px;">⚙️ ПЕРСОНАЛИЗАЦИЯ</button>
        `;
        ...
        document.querySelectorAll('.r-btn').forEach(btn => {
            btn.style.cssText += "color:#fff; border:none; padding:12px; border-radius:8px; font-size:10px; font-weight:bold; cursor:pointer;";
            btn.onclick = async () => {
                ...
                const mySign = getSetting('riv_sign', 'Rivera');
                body += `<br><br>С уважением, ${getSetting('riv_rank', 'Сотрудник')} УМВД — ${getSetting('riv_nick', 'Nick')}.<br>[I]${mySign}[/I][/FONT][/CENTER]`;
