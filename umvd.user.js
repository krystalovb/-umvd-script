...
// @version      22.4
// @description  Full Edition: Авто-детект ника, Корректор, Таймер, Счетчик. (Без FPS Boost)
...
(function() {
    ...
    // --- ФУНКЦИЯ: АВТО-ДЕТЕКТ НИКА ---
    function detectNickname() {
        const lastPost = document.querySelector('.message:last-child .bbWrapper');
        if (lastPost) {
            const text = lastPost.innerText;
            // Ищем формат Nick_Name
    ...
    function createUI() {
        ...
        panel.innerHTML = `
            ...
            ${idleHours && idleHours >= 20 ? `<div style="background:#7f1d1d; color:#fff; font-size:9px; padding:4px; border-radius:5px; text-align:center; margin-bottom:8px; animation: pulse-red 2s infinite;">DEADLINE: ${idleHours}ч.</div>` : ''}
            <button id="btn-ok" style="width:100%; background:#166534; color:white; border:none; padding:10px; border-radius:8px; margin-bottom:5px; font-size:10px; font-weight:bold; cursor:pointer;">ОДОБРИТЬ</button>
            <button id="btn-no" style="width:100%; background:#7f1d1d; color:white; border:none; padding:10px; border-radius:8px; margin-bottom:5px; font-size:10px; font-weight:bold; cursor:pointer;">ОТКАЗАТЬ</button>
            <button id="btn-trans" style="width:100%; background:#1e40af; color:white; border:none; padding:10px; border-radius:8px; margin-bottom:5px; font-size:10px; font-weight:bold; cursor:pointer;">ПЕРЕВОД</button>
            <div id="norm-count" style="text-align:center; font-size:9px; color:#94a3b8; margin:8px 0; border-top:1px solid #333; padding-top:8px;">Норма: 0</div>
            <button id="btn-set" style="width:100%; background:none; border:1px solid #444; color:#94a3b8; font-size:9px; padding:5px; border-radius:6px; cursor:pointer;">⚙️ НАСТРОЙКИ</button>
        `;

        document.body.appendChild(panel);

        document.getElementById('btn-set').onclick = () => {
            const nick = prompt("Ваш Ник:", getSetting('riv_nick', ''));
            ...
            const sign = prompt("Подпись:", getSetting('riv_sign', ''));
    ...
    async function handleResponse(type) {
        ...
        const pNick = prompt("Ник игрока (оставьте как есть, если определен верно):", detected);
        if (!pNick) return;
        ...
        if (type === 'ok') {
            ...
            body += `Ваше заявление на перевод рассмотрено. Вердикт: [COLOR=rgb(34, 197, 94)][B]ОДОБРЕНО[/B][/COLOR] на [B]${rVal}[/B] ранг.`;
        }
