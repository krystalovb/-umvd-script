...
// @version      21.2
// @description  UMVD BY GUDIN: Professional Forum Utility (FPS Boost, Таймер 24ч, Счетчик нормы, Авто-корректор)
// @author       Gudin
...
(function() {
    ...
    // --- FPS BOOST (Оптимизация под Lime) ---
    ...
    function createUI() {
        ...
        panel.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #333; padding-bottom:5px;">
                <div style="color:#fff; font-size:10px; font-weight:bold;">UMVD by Gudin</div>
                <div id="close-panel" style="color:#64748b; cursor:pointer; font-size:18px;">&times;</div>
            </div>
            ${idleHours && idleHours >= 20 ? `<div style="background:#7f1d1d; color:#fff; animation: pulse 1.5s infinite; font-size:9px; padding:5px; border-radius:5px; text-align:center; font-weight:bold;">DEADLINE: ${idleHours}ч.!</div>` : ''}
        ...
        document.getElementById('r-set').onclick = () => showModal({ title: 'SETTINGS', isSettings: true });
            ...
            btn.onclick = async () => {
                ...
                if (type === 'res') {
                    ...
                    if (type === 'ok') body += `Ваше заявление рассмотрено. Вердикт: [B][COLOR=rgb(34, 197, 94)]ОДОБРЕНО[/COLOR][/B].`;
                        ...
                        const curR = await showModal({ title: 'РАНГ', message: 'Текущий ранг (число):' });
                        body += `Ваше заявление на перевод одобрено на [B]${curR - 1}[/B] ранг.`;
                    }
                }
